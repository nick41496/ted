...
void enableRawMode() {
    ...
    raw.c_cflag |= (CS8);
    // BRKINT, INPCK, ISTRIP are miscellaneous flags that are likely already disabled,
    //  but disabling them remains a part of entering raw mode for legacy reasons
    // ICRNL controls carriage return behaviour
    // IXON controls software flow control to stop and resume data transmission (Ctrl-S,
    //  Ctrl-Q)
    raw.c_iflag &= ~(BRKINT | ICRNL | INPCK | ISTRIP | IXON);
    // OPOST controls output processing, \n to \rn translation
    raw.c_oflag &= ~(OPOST);
    // ECHO controls printing what we're typing
    // ICANON controls canonical mode, whether to read line by line or byte by byte
    // IEXTEN controls handling of Ctrl-V, Ctrl-O
    // ISIG controls handling of SIGINT and SIGSTOP (Ctrl-C, Ctrl-Z)
...
void editorFindCallback(char *query, int key) {
    ...
    if (hl) {
        memcpy(E.row[hl_line].hl, hl, E.row[hl_line].rsize);
        free(hl);
        hl = NULL;
    }
...
void editorFind() {
    ...
    char *query = editorPrompt("Search: %s (ESC to cancel/Arrow keys to search back and forth)", 
            editorFindCallback);
        ...
        if (filerow >= E.numrows) {
            if (E.numrows == 0 && y == E.screenrows / 3) {
                ...
                int welcomelen = snprintf(welcome, sizeof(welcome), 
                        "TED (TextEDitor) -- version %s", TED_VERSION);
    ...
    switch (c) {
        ...
        case CTRL_KEY('q'):
            if (E.dirty && quit_times > 0) {
                editorSetStatusMessage("File has unsaved changes. To quit anyway, press "
                        "Ctrl-Q %d more time%s.", quit_times, quit_times == 1 ? "" : "s");
                --quit_times;
                return;
            }
            write(STDOUT_FILENO, "\x1b[2J", 4);
